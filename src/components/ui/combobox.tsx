...
export function Combobox({
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 w-full z-[1100] rounded-lg border border-border bg-white p-0 shadow-[0_8px_32px_rgba(0,0,0,0.12)] animate-in fade-in-0 zoom-in-95 slide-in-from-top-2">
          {/* Search input */}
          ...
          <div className="overflow-y-auto max-h-[60vh] p-1">
            {filteredItems.length === 0 ? (
              <div className="px-3 py-6 text-center text-sm text-white/40">{emptyMessage}</div>
            ) : (
              filteredItems.map(item => (
                ...
                >
                  <span className="absolute left-2.5 flex h-4 w-4 items-center justify-center">
                    {value === item.value && <Check className="h-4 w-4 text-white" />}
                  </span>
